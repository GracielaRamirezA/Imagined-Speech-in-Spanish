...
def load_eeg_data(args, batch_size=32, remove_channel=None):
    ...
    if remove_channel is not None:
        X_train = np.delete(X_train, remove_channel, axis=1)
        X_test = np.delete(X_test, remove_channel, axis=1)
...
def channel_ablation_study(caso):
    ...
    # Probar eliminando cada canal
    for removed_channel in range(16):

        print(f"\n===================================")
        print(f"Eliminando canal {removed_channel}")
        print(f"===================================\n")
        ...
        train_loader, test_loader = load_eeg_data(
            args,
            remove_channel=removed_channel
        )

        best_acc, best_CM = train_raw(
            args,
            model,
            train_loader,
            optimizer,
        ...
        ablation_results[f"Canal_{removed_channel}"] = best_acc

        print(f"Accuracy quitando canal {removed_channel}: {best_acc:.4f}%")

    return ablation_results

caso = 'Multi'

ablation_results = channel_ablation_study(caso)
...
with open(f"ablacion-canales-{caso}.txt", "w", encoding="utf-8") as file:
    json.dump(
        ablation_results,
        file,
        ensure_ascii=False,
